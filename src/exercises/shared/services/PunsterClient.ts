...
export const makePunsterClient = Effect.gen(function* () {
  ...
  const createPun = Effect.fn("PunsterClient.createPun")(
    function* (misbehavior: Misbehavior) {
      const remainingTokens = yield* tokenManager.getBalance(
        misbehavior.childName,
      );
      if (remainingTokens > 0) {
        const remainingTokens = yield* Effect.orDie(
          tokenManager.useToken(misbehavior.childName),
        );
...
function formatSubheader(title: string): AnsiDoc.AnsiDoc {
  return Doc.text(title).pipe(Doc.annotate(Ansi.combine(Ansi.bold, Ansi.blue)));
}
...
function createReport(report: PunDeliveryReport): string {
  ...
  const overallEffectiveness = Doc.catWithSpace(
    Doc.text("Overall Effectiveness").pipe(
      Doc.annotate(Ansi.combine(Ansi.bold, Ansi.magenta)),
    ),
    formatProgressBar(report.metrics.deliveryEffectiveness),
  );
  const comprehensionRate = formatMetric(
    "Comprehension Rate",
    report.metrics.comprehensionRate,
  );
  const reactionIntensity = formatMetric(
    "Reaction Intensity",
    report.metrics.reactionIntensity,
  );
  const reactionDuration = formatMetric(
    "Reaction Duration ",
    Duration.toSeconds(report.metrics.reactionDuration),
  );
  const repeatProbability = formatMetric(
    "Repeat Probability",
    report.metrics.repeatProbability,
  );
  ...
  const footer = Doc.vcat([
    Doc.empty,
    horizontalBar,
    Doc.text(`Report generated at: ${new Date().toISOString()}`).pipe(
      Doc.annotate(Ansi.blackBright),
    ),
    horizontalBar,
  ]).pipe(Doc.annotate(Ansi.combine(Ansi.bold, Ansi.cyan)));
...
function makeLines(
  ...
  const firstLineMaxWidth = config.indentFirstLine
    ? maxWidth - config.indentation
    : maxWidth;
