...
export const makeImmunityTokenManager = Effect.gen(function* () {
  ...
  const getBalance = Effect.fn("ImmunityTokenManager.getBalance")(function* (
    childName: string,
  ) {
  ...
  const awardToken = Effect.fn("ImmunityTokenManager.awardToken")(function* (
    childName: string,
    options: { readonly reason: string },
  ) {
    yield* Effect.log(
      `Awarding immunity token to ${childName} because ${options.reason}`,
    );
    const previous = tokens.get(childName) ?? 0;
    tokens.set(childName, previous + 1);
  });

  const useToken = Effect.fn("ImmunityTokenManager.useToken")(function* (
    childName: string,
  ) {
    return yield* Effect.fromNullable(tokens.get(childName)).pipe(
